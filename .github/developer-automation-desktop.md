...
|---|---|---|---|
| API Picker | `api-picker` | `x0 y0 w6 h24` | Implemented (R2, R5) |
...
- assigns each row its first Swagger `tag` (or `UNTAGGED`) and orders rows **alphabetically by tag** (`localeCompare`, so it is case-insensitive and locale-aware), then by path and method. The contract's declared tag order is deliberately ignored. `UNTAGGED` is a synthetic bucket rather than a real tag, so it always sorts last.

**UI.** Three-level drill-down with a breadcrumb, sized for a 6-column panel. Services are grouped under two collapsible categories, **Favorites then All**, each headed `<Name> (<count>)`; both start expanded and the collapse state is memory-only, matching the operation tag groups. Both headers always render, empty or not, so the list never reflows as the last favorite is removed and the feature is not invisible before first use. There is **no** service count line above the list — the per-category counts replace it. The list is filtered **live** as the user types (no Enter, no search button — the full list is fetched once); Escape in the search box clears it. Matching is all-terms-AND, order-independent, case-insensitive, against the service name only, and is applied to each category independently. Only the `default` category is shown — a service published solely under `partner`/`domain` is invisible. Sorting uses `Intl.Collator(numeric: true, sensitivity: 'base')`, since the API's own order is not numeric-aware. Versions appear in three collapsible sections (**Releases open; Pre-releases and Branches collapsed**) — releases and pre-releases share version names, so a selection is only unique as *(service, category, type, name)*. Operations are grouped under collapsible tag headers, all open by default, with a collapse/expand-all toggle beside the operation count. Clicking an operation **or any of its version chips** selects immediately and publishes to the store.

**Favorite services (R5).** A service belongs to exactly one category. Favorites persist to `localStorage` key `dad-rest-favorites` — its own key, since the per-tab layout blob is geometry only — as a plain sorted name array, size-capped at 64 KB and validated on load, so a corrupt payload degrades to "no favorites" rather than a broken picker. Ordering inside Favorites is alphabetical, never most-recently-added, so a row cannot jump under the cursor.

- **The star slot is absolutely positioned** at the row's right edge with an opaque background tracking the row state (`--c-bg` / `--c-panel-hover` / `--c-active`). A flex sibling would be pushed out of sight by a long service name in a narrow panel; this keeps it inside the panel and painting over the name instead. The name reserves matching right padding, so it ellipsises before it ever reaches the star.
- **`star` at rest, `star-minus` on hover**, swapped by CSS in the one slot, so the width never changes and the always-on marker is never itself clickable. Non-favorite rows show `star-plus` on hover only.
- **The star button suppresses focus** via `onMouseDown` `preventDefault`, not just `tabIndex={-1}`. Chromium focuses a clicked button regardless of tab index, and this one unmounts the instant it is clicked as the row changes category — focus would land outside the panel, clear `focusedInstanceId` and silently disable Tab navigation. Same class of trap as the session list's ✕ button.
- **A row is only labelled `(unavailable)` once a catalogue has actually loaded**, gated on `servicesLoaded` rather than `services.length`. Before the first fetch returns, and after any failure, an empty service list says nothing about whether a favorite still exists — without the gate every favorite would be marked unavailable while off VPN. Such a row stays at `--c-mid` (no text may be dimmer), loses its hover brightening, and does nothing on click, but can still be un-favorited.
- **`servicesLoaded` means "the list in `services` came from a request that succeeded."** It also gates `loadServices`' early return, which previously tested `services.length > 0` — that refetched an empty-but-valid catalogue on every mount and let a stale list block a retry after a failure. `refresh` clears it up front and restores it beside the fetched list, so a failing versions or operations reload further down the same `try` does not discredit a catalogue that loaded fine.
...
- **A response opens with its top level expanded and everything below it collapsed.** `expanded` is a set of ids, so absence means collapsed; `topLevelExpandableIds()` supplies the opening set. That default is discarded in favour of a fully collapsed tree if it would already exceed the 10,000-row budget, so a wide response cannot hang the renderer before the user has asked for anything. It applies only while a tab has no expansion state of its own — collapsing everything by hand sticks.
...
**Layout state validation** (`validateState(value, tabId)`) is tab-aware and **repairing** rather than all-or-nothing. It drops instances whose type the target tab does not allow, re-inserts (via `findSpawnPlacement`) any singleton the tab's default layout requires but the persisted state lacks, preserves/backfills `contentId`, and **coerces any `isGlobal` panel's mode back to `linked`**. It returns `null` only for a payload that is not the instances schema at all. It must never discard a whole tab's layout because one panel is missing.

**Default panel promotion:** When the Default panel of a type is closed, the first remaining **non-global** instance of that type (in reading order) is promoted to Default. It loses its session link but keeps its current content. If only global panels of that type remain, nothing is promoted and the next spawned panel becomes the Default — the same `!inst.isGlobal` test `spawnPanel` uses when it decides a new panel's mode.

**A global panel is never `mode: 'default'`.** The two are contradictory: Default follows the active session, while a global panel is session-unbound and its content deliberately ignores `currentSessionId` (`NotesPanelInstance` keys its scope off `isGlobal`). Promotion used to omit the global check, which badged a global notes panel with the ◆/house Default marker while it correctly went on ignoring session switches — a purely cosmetic lie, but one that persisted to `localStorage`. Both ends are now guarded: promotion cannot create the state, and `validateState` repairs layouts that already hold it.
...
**Saving is group-batched, because validation is.** `ATLASSIAN_PAT` and `ATLASSIAN_BASE_URL` can only be checked *as a pair*, and `validateCredentials` fills any key absent from the update set from the already-stored value. So a per-row save that submits one key is unsaveable on first-time setup: neither value is stored yet, so whichever button is clicked resolves the other half to empty and fails with "Both Access Token and Base URL are required". `handleSaveCred` therefore submits **every pending edit in the clicked field's group** in one `credentials:save` call and applies each per-key result back to its own row. Any future paired credential gets this for free; do not reduce the call back to the single clicked key.
...
- **Rename** a workspace's key by editing it in place in the KEY column (see below).
...
**Renaming a key is a two-store cascade.** The KEY cell is an inline input; `sessions.project` is a *denormalised copy* of the workspace key, driving both the `[ KEY ]` session badge and `hasActiveSession()`, which blocks removal of an in-use workspace. `workspaces:rename` therefore calls `WorkspaceManager.renameWorkspace()` **first** — `workspaces.json` is the source of truth — and only then `SessionManager.reassignProject()`; if the `UPDATE` throws it renames the JSON back, so the two stores can never be left disagreeing. A crash in the window between the two writes is knowingly accepted (worst case: a stale badge and a lost removal guard, both repaired by renaming again); ordering is a wash for crashes, since DB-first loses the same guard. The renderer patches `sessionStore` itself, as the main process emits no event for this. `renameWorkspace()` mutates the entry **in place** so group membership and drag-order survive by construction, and rejects collisions across *all* groups because keys are globally unique.

**Editing UX worth preserving:** blur is the *only* commit path — `Enter` and `Escape` merely blur (Escape sets a revert flag first) — which makes a double commit structurally impossible. Escape also calls `stopPropagation()`, and the dialog's document-level Escape listener additionally bails on `keyDraftRef.current`, or reverting a typo would close the whole dialog; that guard is a **ref** so the listener is not re-registered on every keystroke. The row is `draggable={!isEditing && !isRenaming}`: a `draggable` ancestor swallows click-drag text selection inside the input, and the *pending* half matters too, since the draft clears before the rename resolves and a drag in that window would call `moveWorkspace(oldKey)`, which silently no-ops. This relies on React 18 flushing the discrete focus event synchronously, so draggability drops before `dragstart` can fire.
...
- It reuses `ManageWorkspacesDialog.css` (same pattern as `JiraSettingsDialog`/`NotesSettingsDialog` reusing `CredentialsDialog.css`); only genuinely new rules live in `WorkspaceDiscoveryDialog.css`. The editable KEY input is shared with Manage Workspaces as `.manage-workspaces__key-input`. Its `--invalid` rules are qualified as `.manage-workspaces__input.manage-workspaces__key-input--invalid`: unqualified they tie on specificity with `.manage-workspaces__input`, which also sets `color` and `border`, so the red state would silently depend on source order.
